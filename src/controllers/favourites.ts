  ...
  export const favouritesController = {
    addFavourite: async (req: Request, res: Response) => {
      try {
        ...
        winston.info('User context:', user)
        winston.info('Request params:', req.params)
    ...
    getFavourites: async (req: Request, res: Response) => {
      try {
        ...
        .select()
        .from(favourites)
        .innerJoin(beaches, eq(favourites.beachId, beaches.id))
    ...
    getFavouriteById: async (req: Request, res: Response) => {
      try {
        ...
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
          status: StatusCodes.INTERNAL_SERVER_ERROR,
          message: 'An unexpected error occurred while fetching the favorite beach'
        })
      }
    }
  }